...
public final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
    ...
    /**
     * Walks an AVL tree in iteration order. Once a node has been returned, its left, right and parent links are <strong>no longer used</strong>. For this reason it is safe to transform these links as you walk a tree.
     *
     * <p>
     * <strong>Warning:</strong> this iterator is destructive. It clears the parent node of all nodes in the tree. It is an error to make a partial iteration of a tree.
     */
     ...
     *
     * <p>
     * The returned tree will satisfy the AVL constraint: for every node <i>N</i>, the height of <i>N.left</i> and <i>N.right</i> is different by at most 1. It accomplishes this by omitting deepest-level leaf nodes when building trees whose size isn't a power of 2 minus 1.
     *
     * <p>
     * Unlike rebuilding a tree from scratch, this approach requires no value comparisons. Using this class to create a tree of size <i>S</i> is {@code O(S)}.
     */
    final static class AvlBuilder<K, V> {
        ...
        void add(Node<K, V> node) {
            ...
            /*
             * Combine 3 nodes into subtrees whenever the size is one less than a multiple of 4. For example we combine the nodes A, B, C into a 3-element tree with B as the root.
             *
             * Combine two subtrees and a spare single value whenever the size is one less than a multiple of 8. For example at 8 we may combine subtrees (A B C) and (E F G) with D as the root to form ((A B C) D (E F G)).
             *
             * Just as we combine single nodes when size nears a multiple of 4, and 3-element trees when size nears a multiple of 8, we combine subtrees of size (N-1) whenever the total size is 2N-1 whenever N is a power of 2.
             */
